...
// Declared here rather than in png_dec.h so this file stays self-contained; slaunch.c
// carries its own declaration. (If you'd rather put it in include/png_dec.h, drop this
// line and slaunch.c's - two declarations in one translation unit trip -Wredundant-decls.)
void png_dec_finalize(void);

/***********************************************************************
* persistent decoder
*
* Create/Destroy is the session-level pair in this API; Open/Close is the per-file pair.
* The stock code ran Create/Destroy around every image, so drawing a page of the grid set
* the decoder up and tore it down ten times over (forty in the 10x4 grid).
*
* This is a smaller win than the same change in jpg_dec.c: PNG decoding here is PPU-only
* (see spuThreadEnable below), so there is no SPU thread being started and stopped each
* time - just the decoder object and its callback allocations. Worth having anyway, and
* it is what the API expects.
***********************************************************************/
static png_dec_info png_dec;
static uint8_t      png_dec_ready = 0;

static int32_t open_decoder(void)
{
	if(png_dec_ready) return CELL_OK;

	if(create_decoder(&png_dec) != CELL_OK) return -1;
...
void png_dec_finalize(void)
{
	if(!png_dec_ready) return;

	PngDecDestroy(png_dec.main_h);
	png_dec_ready = 0;
}
...
Buffer load_png(const char *file_path, void* buf_addr)
{
	...
	{
		// open png stream
		if(open_png(&png_dec, file_path)==CELL_OK)
		{
			// set decode parameter
			if(set_dec_param(&png_dec)==CELL_OK)
			{
				// decode png stream, into target buffer
				decode_png_stream(&png_dec, buf_addr);
				tmp.w = png_w;
				tmp.h = png_h;
			}

			// close png stream
			PngDecClose(png_dec.main_h, png_dec.sub_h);
		}
	}
